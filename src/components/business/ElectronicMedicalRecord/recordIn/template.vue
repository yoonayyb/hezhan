<template>
  <el-collapse v-model="activeNames" v-loading="loadStatus">
    ...
    <el-collapse-item title="转科目的及注意事项：" name="7" :disabled="true">
      <div>
        内容：
        <span v-text="data.trans_dia_plan"></span>
      </div>
    </el-collapse-item>
...
import { getRecordIn } from '@/api/modules/medicalRecord.js'
export default {
  mixins: [mixTempla],
  props: ['id'],
  data () {
  ...
  methods: {
    async getData () {
      const { data } = await getRecordIn(this.emrID)
      this.data = data || {}
